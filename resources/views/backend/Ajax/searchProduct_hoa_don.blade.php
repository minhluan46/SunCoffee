@if (isset($SanPham))
    @foreach ($SanPham as $item)
        <tr>
            <td><img src="{{ asset('uploads/SanPham/' . $item->hinhanh) }}" alt="" height="60">
                <p class="d-inline-block align-middle mb-0 f_s_16 f_w_600 color_theme2" style="padding-left: 10px">
                    {{ $item->tensanpham }}</p>
            </td>
            <td>
                <select onchange="kichthuoc(this,'{{ $item->id }}')" data-ia="{{ $item->id }}"
