@if (isset($NhanVien))
    @foreach ($NhanVien as $value)
        <tr id="{{ $value->id }}">
            ...
            <td>
                <a onclick="Show('{{ route('nhan-vien.show', $value->id) }}')" href="javascript:(0)"
                    class="action_btn mr_10"><i class="fas fa-eye"></i></a>
                <a href="{{ route('nhan-vien.edit', $value->id) }}" class="action_btn mr_10"><i
                        class="fas fa-edit"></i></a>
                <a data-url="{{ route('nhan-vien.destroy', $value->id) }}" data-id="{{ $value->id }}"
                    href="javascript:(0)" class="action_btn form-delete"><i class="fas fa-trash-alt"></i></a>
            </td>
        </tr>
    @endforeach
@endif
