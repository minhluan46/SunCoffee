...
<body>
    ...
    <p style="margin: 0;margin-bottom: 15px;">Vào lúc {{ Date_format(Date_create($DanhGia->thoigian), 'd/m/Y H:i:s') }} bạn đã đánh giá sản phẩm
        ...
        <a style=" text-decoration: none; color: blue" href="{{ route('Trangchu.index') }}">Sun Coffee</a> với {{ $DanhGia->sosao }} sao và nội dung là: "{{ $DanhGia->noidung }}"
    </p>
    <p style="margin: 15px 0px 35px 0px;">Chúng tôi thật tiết khi nội dung đánh giá của bạn đã vi phạm quy định đăng đánh giá của chúng tôi.</p>
    ...
    <p style="margin: 7px 0px;">- Thông tin ảnh hưởng đến công việc kinh doanh của <a style=" text-decoration: none; color: blue" href="{{ route('Trangchu.index') }}">Sun Coffee</a>.</p>
    <p style="margin: 7px 0px 35px 0px;">- Những thông tin khác mà công ty chúng tôi cho rằng không hợp lý khi đăng tải lên <a style=" text-decoration: none; color: blue"
            href="{{ route('Trangchu.index') }}">Sun Coffee</a>.</p>
