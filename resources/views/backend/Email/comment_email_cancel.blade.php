...
<body>
    ...
    <p style="margin: 0;margin-bottom: 15px;">Vào lúc {{ Date_format(Date_create($TimBinhLuan->thoigian), 'd/m/Y H:i:s') }} bạn đã bình luận tại
        <a style=" text-decoration: none; color: blue" href="{{ route('Trangchu.index') }}">Sun Coffee</a> với nội dung: "{{ $TimBinhLuan->noidung }}"
    </p>
        ...
        <div id="listreply">
            @isset($TraLoi)
                @foreach ($TraLoi as $item)
                    <div class="rowuser">
                        @if ($item->trangthai == 2)
                            <img src="{{ asset('uploads/Logo/logo_sun_coffee.png') }}" alt="" style="width: 25px">
                        @else
