        ...
        <div class="col-lg-12">
            ...
            <h4 style="text-align: center">
                @isset($ChiTietSanPham)
                    @foreach ($ChiTietSanPham as $itemctsp)
                        @if ($itemctsp->id == $ChiTietKhuyenMai->id_chitietsanpham)

                            @isset($SanPham)
                                @foreach ($SanPham as $itemsp)
                                    @if ($itemsp->id == $itemctsp->id_sanpham)
                                        {{ $itemsp->tensanpham }} {{ '( ' . $itemctsp->kichthuoc . ' )' }}
                                    @endif
                                @endforeach
                            @endisset
                        @endif
            ...
            <div class="form-group">
                ...
                <input type="number" class='form-control' name="muckhuyenmai"
                    value="{{ $ChiTietKhuyenMai->muckhuyenmai }}">
            </div>
            <div class="form-group">
                <label>Giá Khuyến Mãi<b style="color:red"> *</b></label>
                <input type="number" class='form-control money' name="giakhuyenmai"
                    value="{{ $ChiTietKhuyenMai->giakhuyenmai }}">
            </div>
            <button id="form-edit-CTKM" class="btn btn-success" type="submit" style="width: 100%"
                data-idctsp="{{ $ChiTietKhuyenMai->id_chitietsanpham }}"
